...
void Concentration::compute_phi(unsigned int step, Eigen::VectorXd& psi1, Eigen::VectorXd& psi2, Eigen::VectorXd& psi3, Eigen::VectorXd& psi4, Eigen::VectorXd& psi5) const
{
    psi1=Ca.col(step);
...
void Concentration::output_results_fixed_time(const std::string& name) const
{
    ...
    for (unsigned int i = 0; i<data_transp.Nx; ++i) //Loop to save the matrix by column in the CSV file
    {
        ...
        for (unsigned int j=0; j<data_transp.Nt; j+=a)
        {
            file1<<value1(i,j)<<", ";
        }

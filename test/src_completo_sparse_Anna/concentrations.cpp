...
void Concentration::compute_phi(unsigned int step, Eigen::VectorXd& phi1, Eigen::VectorXd& phi2, Eigen::VectorXd& phi3, Eigen::VectorXd& phi4)
{
  phi1=Ca.col(step);
...
void Concentration::compute_rd(unsigned int step, Eigen::VectorXd& rd){
  ...
  for (unsigned int i=0; i<Ca.rows(); ++i)
   {   
       ...
       omega=Ca(i,step)*Ca(i,step)/(H_piu(i,step)*H_piu(i,step));//nel codice di Anna c'è anche un Hpiu al quadrato
 ...
 switch(method)
     { case 1: //Euler Esplicit  
       ...
       case 2: //Predictor Corrector
       {
        ...
        Euler_Esplicit(phi1_,phi2_,phi3_,phi4_,rd,rhs,rhs_CO2,solver,solver2);
        compute_concentration(step,phi1_,phi2_,phi3_,phi4_);
...
void Concentration::Euler_Esplicit(Eigen::VectorXd& phi1, Eigen::VectorXd& phi2, Eigen::VectorXd& phi3, Eigen::VectorXd& phi4, const Eigen::VectorXd& rd, const Eigen::SparseMatrix<double>& rhs, const Eigen::VectorXd& rhs_CO2, Eigen::SparseLU<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int> > &solver, Eigen::SparseLU<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int> > &solver2)
{ 
  ...
  //transport_and_reaction(phi3,rhs,Eigen::VectorXd::Zero(phi2.size()),solver);
...
void Concentration::transport_and_reaction(Eigen::VectorXd& phi, const Eigen::SparseMatrix<double>& rhs, const Eigen::VectorXd& rd,Eigen::SparseLU<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int> > &solver){
     ...
     double h{static_cast<double>(data_transp.L/data_transp.Nx)};
     const Eigen::VectorXd temp=rhs*phi+rd*h;
...
{
 double h{static_cast<double>(data_transp.L/data_transp.Nx)}; 
 const Eigen::VectorXd temp=rhs*phi+rhs_CO2+rd*h;
...
void Concentration::compute_concentration(unsigned int step, const Eigen::VectorXd& phi1, const Eigen::VectorXd& phi2, const Eigen::VectorXd& phi3, const Eigen::VectorXd& phi4){
  ...
  for(unsigned int i=0; i<Ca.rows(); ++i)
  {
   ...
   Eigen::VectorXd dx(5);
      ...
      for(unsigned int iter=0; iter<max_iter and err>tol; ++iter)
       {
        compute_rhs(rhs, old_it, phi1(i), phi2(i), phi3(i), phi4(i));///Calcolo F(x_k);
...
void Concentration::compute_rhs(Eigen::VectorXd& rhs,const Eigen::VectorXd& old_it, double phi1, double phi2, double phi3, double phi4){
  
   double Ca{old_it(0)}, H_piu{old_it(1)}, HCO3_meno{old_it(2)}, CO2{old_it(3)}, CaSiO3{old_it(4)};
   rhs<<Ca-phi1,H_piu-HCO3_meno-phi2,CO2+HCO3_meno-phi3,CaSiO3-phi4,H_piu*HCO3_meno-data_reagents.K_eq*CO2;
}

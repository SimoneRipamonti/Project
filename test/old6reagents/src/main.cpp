...
int main()
{
    ...
    Eigen::VectorXd vel{sol.head(data.Nx+1)};
...
//Setting for transport part of the equation
    Eigen::SparseMatrix<double> M_solid(Nx,Nx);//Mass matrix for the transport part
    Eigen::SparseMatrix<double> rhs_solid(Nx,Nx);//rhs for the transport part
    concentration.assemble_transport_solid(M_solid,rhs_solid,Eigen::VectorXd::Zero(Nx+1));//function that assembles the transport part
...
//Here solver for the transport system is initialized
    Eigen::SparseLU<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int> >   solver3;
    solver3.analyzePattern(M_solid);
    solver3.factorize(M_solid);
...
//Loop Temporale
    ...
    for(unsigned int i=1; i<Nt+1; i++)
     {
       ...
       concentration.compute_rd_kp(i-1,rd);//Calcolo i termini di reazione
        ...
        concentration.one_step_transport_reaction(psi1,psi2,psi3,psi4,psi5,rd,rhs,rhs_CO2,i,solver,solver2,solver3);
    ...
    concentration.output_results_fixed_space("H_piu");
    concentration.output_results_fixed_space("HCO3_meno");
    concentration.output_results_fixed_space("CO2");
    concentration.output_results_fixed_space("CaSiO3");
    concentration.output_results_fixed_space("SiO2"); 

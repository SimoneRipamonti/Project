...
{ 
   ...
   solver.factorize(M);     
    ...
    {
        rhs=(1/dt*C.get_matrix()-F_p.get_matrix()+F_m.get_matrix()-lambda*Reaction*h)*Ca.col(i-1)+C.get_rhs();
...
{ 
    ...
    const Eigen::SparseMatrix<double>  M{C.get_matrix()+dt*F_p.get_matrix()-dt*F_m.get_matrix()};
   ...
   solver.factorize(M);  
    ...
    for(unsigned int i=1; i<Nt; i++)
    {
        rhs=(C.get_matrix()-dt*lambda*Reaction*h)*Ca.col(i-1)+dt*C.get_rhs();//Bisognerebbe moltiplicarlo per la porosità il termine reattivo
        if(i==1)
